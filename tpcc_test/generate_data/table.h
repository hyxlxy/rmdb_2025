...
void write_sql_into_file(std::string sql_path, std::vector<std::string> sql)
{
    std::ofstream outfile;
    outfile.open(sql_path, std::ios::out | std::ios::trunc);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
            std::string create_tb="create table warehouse (w_id int, w_name char(10), w_street_1 char(20), w_street_2 char(20), w_city char(20), w_state char(2), w_zip char(9), w_tax float, w_ytd float);";
        sqls.push_back(create_tb);
        generate_table_data();
        write_sql_into_file(sql_path, sqls);
    }
...
public:
    ...
    {
            std::string create_tb="create table district (d_id int, d_w_id int, d_name char(10), d_street_1 char(20), d_street_2 char(20), d_city char(20), d_state char(2), d_zip char(9), d_tax float, d_ytd float, d_next_o_id int);";
        sqls.push_back(create_tb);
        d_w_id = w_id;
        d_ytd = 3000.5;
        // d_next_o_id = 3001;
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
           std::string create_tb="create table customer (c_id int, c_d_id int, c_w_id int, c_first char(16), c_middle char(2), c_last char(16), c_street_1 char(20), c_street_2 char(20), c_city char(20), c_state char(2), c_zip char(9), c_phone char(16), c_since char(30), c_credit char(2), c_credit_lim int, c_discount float, c_balance float, c_ytd_payment float, c_payment_cnt int, c_delivery_cnt int, c_data char(50));";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
               std::string create_tb="create table history (h_c_id int, h_c_d_id int, h_c_w_id int, h_d_id int, h_w_id int, h_date char(19), h_amount float, h_data char(24));";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
                   std::string create_tb="create table new_orders (no_o_id int, no_d_id int, no_w_id int);";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
                       std::string create_tb="create table orders (o_id int, o_d_id int, o_w_id int, o_c_id int, o_entry_d char(19), o_carrier_id int, o_ol_cnt int, o_all_local int);";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
                           std::string create_tb="create table order_line (ol_o_id int, ol_d_id int, ol_w_id int, ol_number int, ol_i_id int, ol_supply_w_id int, ol_delivery_d char(19), ol_quantity int, ol_amount float, ol_dist_info char(24));";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
                               std::string create_tb="create table item (i_id int, i_im_id int, i_name char(24), i_price float, i_data char(50));";
        sqls.push_back(create_tb);
        generate_table_data();
        write_sql_into_file(sql_path, sqls);
    }
...
public:
    ...
    void generate_insert_sql(std::string sql_path)
    {
                                   std::string create_tb="create table stock (s_i_id int, s_w_id int, s_quantity int, s_dist_01 char(24), s_dist_02 char(24), s_dist_03 char(24), s_dist_04 char(24), s_dist_05 char(24), s_dist_06 char(24), s_dist_07 char(24), s_dist_08 char(24), s_dist_09 char(24), s_dist_10 char(24), s_ytd float, s_order_cnt int, s_remote_cnt int, s_data char(50));";
        sqls.push_back(create_tb);
        for (int i = 1; i <= NUM_WARE; ++i)
        {
            generate_table_data(i);
