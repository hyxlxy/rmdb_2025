 ...
 * @note 修复：范围应该从0开始，与lower_bound保持一致
 */
                                                              ...
                                                              Transaction *transaction, bool find_first)
{
    ...
    if (!node)
    {
        throw std::runtime_error("Failed to fetch root node");
    }

    while (!node->is_leaf_page())
    {
        auto child_page_no = find_first ? node->value_at(0) : node->internal_lookup(key);
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction)
{
    ...
    // key 重复 - 对于唯一索引，返回IX_NO_PAGE让上层处理
    ...
    if (new_size == old_size)
    {
        ...
        // 对于唯一索引，重复键返回IX_NO_PAGE让create_index函数处理
        return IX_NO_PAGE;
    }
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction)
{
    ...
    IxNodeHandle *leaf_node = nullptr;
    bool is_root_locked = false;
    
    try {
        auto result = find_leaf_page(key, Operation::DELETE, transaction, false);
        leaf_node = result.first;
        is_root_locked = result.second;
        ...
        if (pos == 0 && leaf_node->get_size() > 0)
        {
            maintain_parent(leaf_node);
        }

        // 检查是否需要合并或重分配
        bool node_should_be_deleted = coalesce_or_redistribute(leaf_node, transaction, &is_root_locked);

        // 如果节点需要被删除，记录到事务的删除页面集合中
        if (node_should_be_deleted && transaction != nullptr)
        {
            // 修复：确保页面指针有效后再添加到删除集合
            if (leaf_node && leaf_node->page) {
                transaction->append_index_deleted_page(leaf_node->page);
            }
        }

        // 解锁和 unpin - 修复：确保页面ID有效
        if (leaf_node && leaf_node->get_page_id().page_no != INVALID_PAGE_ID) {
            buffer_pool_manager_->unpin_page(leaf_node->get_page_id(), true);
        }
        ...
        {
            // 注意：实际的页面删除应该在事务提交时进行；
            // 这里不再直接物理删除页面，以避免删除错误页面或仍在使用的页面导致崩溃。
            // 由 coalesce()/adjust_root 已维护父子/兄弟指针与元信息，物理删除延迟到更安全的时机。
            (void)transaction; // 保留接口形参
        }
...
bool IxIndexHandle::adjust_root(IxNodeHandle *old_root_node)
{
    ...
    if (old_root_node->is_internal_page())
    {
        ...
        else if (old_root_node->get_size() == 0)
        {
            // 修复：内部根节点大小为0的异常情况，重置为初始根页面
            file_hdr_->root_page_ = IX_INIT_ROOT_PAGE;
            // 修复：当内部根节点被删除时，也需要重置叶子链表指针
            file_hdr_->first_leaf_ = IX_NO_PAGE;
            file_hdr_->last_leaf_ = IX_NO_PAGE;
            return true;
        }
                             ...
                             Transaction *transaction, bool *root_is_latched)
{
    ...
    // 如果有事务，将要删除的节点添加到事务的删除页面集合中
    if (transaction != nullptr && node_ && node_->page)
    {
        // 修复：确保页面指针有效后再添加到删除集合
    ...
    {
        // 如果父节点是根节点且大小为0，需要特殊处理
        return adjust_root(*parent);
    }
    else if ((*parent)->get_size() < (*parent)->get_min_size() && !(*parent)->is_root_page())
    {
        // 如果父节点不满足最小大小要求且不是根节点，需要递归处理
...
bool IxIndexHandle::coalesce_or_redistribute(IxNodeHandle *node, Transaction *transaction, bool *root_is_latched)
{
    ...
    if (node->is_root_page())
    {
        ...
        // 根节点调整完成：释放根锁
        if (root_is_latched != nullptr && *root_is_latched)
        {
            root_latch_.unlock();
    ...
    try
    {
        parent_node = fetch_node(node->get_parent_page_no());
        if (!parent_node) {
            // 修复：父节点获取失败时的处理
            return false;
        }
        
        index = parent_node->find_child(node);

        // 检查index是否有效（parent_node->get_size() 为num_key，child索引范围[0, num_key)）
        if (index < 0 || index >= parent_node->get_size())
        {
        ...
        // neighbor_index 同样应在[0, num_key)
        if (neighbor_index < 0 || neighbor_index >= parent_node->get_size())
        {
            buffer_pool_manager_->unpin_page(parent_node->get_page_id(), false);
            return false;
        }

        neighbor_node = fetch_node(parent_node->value_at(neighbor_index));
        if (!neighbor_node) {
            // 修复：兄弟节点获取失败时的处理
            buffer_pool_manager_->unpin_page(parent_node->get_page_id(), false);
            return false;
 ...
 * @deprecated 建议使用IxScan::rid()方法，它有更好的缓存机制
 */
Rid IxIndexHandle::get_rid(const Iid &iid) const
{
    // 检查页面号是否有效
    if (iid.page_no == IX_NO_PAGE || iid.page_no < 0)
    {
        throw IndexEntryNotFoundError();
    }
...
Iid IxIndexHandle::leaf_end() const
{
    ...
    {
        return {.page_no = IX_NO_PAGE, .slot_no = 0};
    }
...
IxNodeHandle *IxIndexHandle::fetch_node(int page_no) const
{
    ...
    if (!page)
    {
        throw std::runtime_error("Failed to fetch page " + std::to_string(page_no) + " from buffer pool");
    }
...
IxNodeHandle *IxIndexHandle::create_node()
{
    ...
    // 让BufferPoolManager分配页面号，然后更新file_hdr_
...
void IxIndexHandle::maintain_parent(IxNodeHandle *node)
{
    ...
    while (curr->get_parent_page_no() != IX_NO_PAGE)
    {
        ...
        curr = parent;
        // 注意：这里不unpin parent，因为它现在是curr
    }

    // 修复：确保最后的curr也被unpin（如果它不是原始node且到达了根节点）
    if (curr != node && curr->get_parent_page_no() == IX_NO_PAGE)
    {
        buffer_pool_manager_->unpin_page(curr->get_page_id(), true);
    }
}
...
// 批量插入接口实现
void IxIndexHandle::insert_entries(const char *key_buf, int key_size, const Rid *rid_buf, size_t n, Transaction *transaction)
{
    // 先将key-rid对排序后顺序插入，减少B+树分裂和提升cache命中率
    struct KeyRid
    {
        std::vector<char> key;
        Rid rid;
        KeyRid(const char *k, int key_size, const Rid &r) : key(k, k + key_size), rid(r) {}
    };
    ...
    {
        pairs.emplace_back(key_buf + i * key_size, key_size, rid_buf[i]);
        if (is_sorted && i > 0)
        {
            if (ix_compare(pairs[i - 1].key.data(), pairs[i].key.data(), file_hdr_->col_types_, file_hdr_->col_lens_) > 0)
            {
                is_sorted = false;
            }
        }
    ...
    if (!is_sorted)
    {
        std::sort(pairs.begin(), pairs.end(), [&](const KeyRid &a, const KeyRid &b)
                  { return ix_compare(a.key.data(), b.key.data(), file_hdr_->col_types_, file_hdr_->col_lens_) < 0; });
    }
    for (size_t i = 0; i < n; ++i)
    {
        insert_entry(pairs[i].key.data(), pairs[i].rid, transaction);
    }
}