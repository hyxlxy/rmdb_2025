...
public:
    ...
    void destroy_index(const std::string &index_name) {
        // 如果文件仍然打开，先关闭它
        if (disk_manager_->is_file_open(index_name)) {
            try {
                int fd = disk_manager_->get_open_file_fd(index_name);
                disk_manager_->close_file(fd);
    ...
    void destroy_index(const std::string &filename, const std::vector<ColMeta> &index_cols) {
        ...
        // 如果文件仍然打开，先关闭它
        if (disk_manager_->is_file_open(ix_name)) {
            try {
                int fd = disk_manager_->get_open_file_fd(ix_name);
                disk_manager_->close_file(fd);
            } catch (const FileNotOpenError&) {
                // 忽略，文件可能已经被关闭
            }
