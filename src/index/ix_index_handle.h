...
public:
    ...
    void insert_entries_batch(const char *key_buf, int key_size, const Rid *rid_buf, int count, Transaction *transaction);
    ...
    {
        // 线性扫描（与内部实现一致，children存储在rids[0..num_key-1]）
        int rid_idx;
...
public:
    ...
    // 批量插入接口
    void insert_entries(const char *key_buf, int key_size, const Rid *rid_buf, size_t n, Transaction *transaction);
