...
public:
    ...
    // 添加新的构造函数，用于事务回滚时创建插入日志记录
    InsertLogRecord(txn_id_t txn_id, Rid& rid, std::string table_name, RmRecord& insert_value)
        : InsertLogRecord() {
        log_tid_ = txn_id;
...
{public:
    ...
    // 添加新的构造函数，用于事务回滚时创建删除日志记录
    DeleteLogRecord(txn_id_t txn_id, Rid& rid, std::string table_name, RmRecord& delete_value)
        : DeleteLogRecord() {
        log_tid_ = txn_id;
        ...
        // 这两个字段可能需要从文件头中获取，但在回滚时可能不需要
        // 暂时设置为默认值
        first_free_page_no_ = -1;
        log_tot_len_ += sizeof(int);
        num_pages_ = -1;
        log_tot_len_ += sizeof(int);
    }
