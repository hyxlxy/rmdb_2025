...
bool BufferPoolManager::find_victim_page(frame_id_t* frame_id) {
    ...
    if(free_list_.size() >0){//空闲帧的帧号列表>0:如果有空闲帧
        *frame_id = free_list_.front();//将空闲帧列表的第一个赋给参数
        free_list_.pop_front();//将该空闲帧删除
        return true;
    }
    else{//如果没有空闲帧
        return replacer_->victim(frame_id);//找到可以替换的页面,LRUlist末尾的帧
    }
...
void BufferPoolManager::update_page(Page *page, PageId new_page_id, frame_id_t new_frame_id) {
    ...
    page_table_.erase(page->get_page_id());//删除原有page的page_table表项
    page_table_[new_page_id]=new_frame_id;//插入新的表项
    page->reset_memory();//数据区全部清零，重置该页的内存内容
...
Page* BufferPoolManager::fetch_page(PageId page_id) {
    ...
    if(iter != page_table_.end()){//如果在pagetable中
        ...
        replacer_->pin(frame_id);//从LRUlist中将这个帧移除，将该帧固定
        Page *page = &pages_[frame_id];
        page->pin_count_++;//使用该页面的线程加一
        return page;
    }
    else{//如果page_id对应的page不在pagetable中
        frame_id_t frame_id;
...
bool BufferPoolManager::unpin_page(PageId page_id, bool is_dirty) {
    ...
    auto &pin_count = pages_[frame_id].pin_count_;
    if (pin_count == 0) {
        return false;
    }
    --pin_count;
