...
class LockManager {
    ...
    class LockRequestQueue {
        ...
        // 禁用拷贝构造和赋值（因为condition_variable不可拷贝）
        LockRequestQueue(const LockRequestQueue&) = delete;
        LockRequestQueue& operator=(const LockRequestQueue&) = delete;

        // 允许移动构造和赋值
        LockRequestQueue(LockRequestQueue&&) = default;
        LockRequestQueue& operator=(LockRequestQueue&&) = default;
    };
