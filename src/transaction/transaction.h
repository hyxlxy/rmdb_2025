...
public:
        ...
        : state_(TransactionState::DEFAULT), isolation_level_(isolation_level), txn_id_(txn_id)
    {
        ...
        // **关键修复：初始化事务模式为隐式事务（自动提交）**
        txn_mode_ = false; // false表示隐式事务，true表示显式事务

        // MVCC相关初始化
        read_ts_ = INVALID_TIMESTAMP;
        commit_ts_ = INVALID_TIMESTAMP;
        snapshot_ = nullptr;
    }
    ...
    inline timestamp_t get_read_ts() { return read_ts_; }

    inline void set_commit_ts(timestamp_t commit_ts) { commit_ts_ = commit_ts; }
    inline timestamp_t get_commit_ts() { return commit_ts_; }

    inline bool is_committed() { return commit_ts_ != INVALID_TIMESTAMP; }
    inline bool is_active() { return state_ == TransactionState::GROWING || state_ == TransactionState::SHRINKING; }

    // 撤销日志相关方法
    inline std::vector<UndoRecord> &get_undo_logs() { return undo_logs_; }
    inline void add_undo_log(const UndoRecord &undo_record) { undo_logs_.push_back(undo_record); }

    // 快照相关方法
    inline void set_snapshot(void *snapshot) { snapshot_ = snapshot; }
    inline void *get_snapshot() { return snapshot_; }
    ...
    void commit()
    {
        ...
        for (auto write_record : *write_set_)
        {
            ...
            {
                // 这里应该只更新MVCC版本的时间戳，不删除物理记录
            }
        }

        // 更新MVCC版本的时间戳
        // Get commit timestamp from transaction manager (needs to be passed as parameter)
        // timestamp_t commit_ts = get_next_timestamp(); // This line causes the error
        // The commit timestamp should be set by TransactionManager::commit_transaction()
        commit_ts_ = commit_ts_; // Use the already set commit timestamp
...
private:
    ...
    // MVCC相关字段
    timestamp_t read_ts_;               // 事务的读取时间戳（快照时间戳）
    timestamp_t commit_ts_;             // 事务的提交时间戳
    std::vector<UndoRecord> undo_logs_; // MVCC撤销日志
    void *snapshot_;                    // 事务的快照（使用void*避免前向声明问题）
