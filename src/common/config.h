...
// replacer
static const std::string REPLACER_TYPE = "CLOCK"; // 表文件 buffer 使用 CLOCK replacer

static const std::string DB_META_NAME = "db.meta";

// 定义静态检测点的全局变量
static const std::string CHECK_POINT_NAME = "db.check_point";

// MVCC控制开关
static constexpr bool ENABLE_MVCC = true;  // 启用MVCC

// TPCC性能优化开关
static constexpr bool TPCC_PERFORMANCE_MODE = true; // 启用TPCC性能优化模式
static constexpr bool DISABLE_DEBUG_OUTPUT = true;  // 禁用调试输出以提升性能
static constexpr bool TPCC_FAST_MODE = false;       // 关闭超快模式，避免跳过关键MVCC检查

// TPCC一致性优化
static constexpr bool TPCC_CONSISTENCY_MODE = true;      // 启用TPCC一致性优化模式
static constexpr bool STRICT_SNAPSHOT_ISOLATION = false; // 先保持非严格SI，性能更稳；需要更严格可改为true
