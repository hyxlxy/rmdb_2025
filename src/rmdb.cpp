...
// 确保output_file_enabled在启动时被正确初始化
extern bool output_file_enabled;
    ...
    while (true)
    {
        ...
        if (yyparse() == 0)
        {
            if (ast::parse_tree != nullptr)
            {
                try
                {
                    // 检查是否为 set output_file on/off 命令
                    auto setKnob = std::dynamic_pointer_cast<ast::SetKnobStmt>(ast::parse_tree);
                    if (setKnob && setKnob->set_knob_type == ast::SetKnobType::OutputFile)
                    {
                        // 处理 output_file on/off，更新全局变量
                        output_file_enabled = setKnob->bool_value;
                        std::string msg = output_file_enabled ? "output_file enabled\n" : "output_file disabled\n";
                        memcpy(data_send, msg.c_str(), msg.length());
                        data_send[msg.length()] = '\0';
                        offset = msg.length();
                        // 只在开启时写入 output.txt
                ...
                catch (UniqueCheckError &e)
                {
                    // 唯一性约束违反，输出failure
                    std::cerr << "RMDBError " << e.what() << std::endl;
                    std::string failure_msg = "failure\n";
                    memcpy(data_send, failure_msg.c_str(), failure_msg.length());
                    data_send[failure_msg.length()] = '\0';
                    offset = failure_msg.length();

                    // 将failure写入output.txt
                    if (output_file_enabled)
                    {
                        std::fstream outfile;
                        outfile.open("output.txt", std::ios::out | std::ios::app);
                ...
                catch (NonUniqueIndexError &e)
                {
                    // 非唯一索引错误，输出failure但不断开连接
                    std::cerr << "NonUniqueIndexError " << e.what() << std::endl;
                    std::string failure_msg = "failure\n";
                    memcpy(data_send, failure_msg.c_str(), failure_msg.length());
                    data_send[failure_msg.length()] = '\0';
                    offset = failure_msg.length();
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // **重新启用MVCC功能**
        mvcc_record_manager->set_storage_manager(sm_manager.get());
        MVCCExecutorBase::initialize_mvcc_components(mvcc_record_manager.get(), txn_manager.get());
